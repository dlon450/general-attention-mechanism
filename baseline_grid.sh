...
OUT="$ROOT/baseline_results"; mkdir -p "$OUT"
STEPS="${STEPS:-4000}"
F2S=(mha sparsemax modular rep_key rep_val)
SEEDS=(0 1 2)
...
for task in needle redundancy; do
  for f in "${F2S[@]}"; do
    for s in "${SEEDS[@]}"; do
      ...
      cmd="CUDA_VISIBLE_DEVICES=$gpu $V $ROOT/f2_sweep.py --task $task --f2 $f --L 512 --steps $STEPS --seed $s > $OUT/$name.json 2> $OUT/$name.err"
