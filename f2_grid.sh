...
OUT="$ROOT/f2_results"; mkdir -p "$OUT"
STEPS="${STEPS:-4000}"

F2S=(mha modular card rep_key rep_val submod)
SEEDS=(0 1 2)
# task:extra
...
for task in needle redundancy; do
  ...
  for f in "${F2S[@]}"; do
    for s in "${SEEDS[@]}"; do
      ...
      cmd="CUDA_VISIBLE_DEVICES=$gpu $V $ROOT/f2_sweep.py --task $task --f2 $f --L $L --steps $STEPS --seed $s > $OUT/$name.json 2> $OUT/$name.err"
